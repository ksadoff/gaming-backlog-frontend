...
export default function LibrariesPage() {
  ...
  const [filteredLibraries, setFilteredLibraries] =
    useState<Array<LibraryPreview>>(userLibraries);
  ...
  const gamesToPreviews = (games: Array<GameInstance>) => {
    ...
    games.forEach((game) =>
      gamePreviews.push({ id: game.id, name: game.name }),
    );
    return gamePreviews;
  };
  ...
  useEffect(() => {
    const fetchLibraries = async () => {
      ...
      libraries.forEach((library) =>
        libraryPreviews.push({
          id: library.id,
          name: library.name,
          games: gamesToPreviews(library.games),
        }),
      );
      ...
      <div>
        {filteredLibraries.map((preview) => {
          return (
            <div key={preview.id}>
              <h1 style={{ zIndex: 10 }}>
                <a href={`${homeUiUrl}` + "libraries/" + preview.id}>
                  {preview.name}
                </a>
              </h1>
